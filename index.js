...
// Handle callback queries - FIXED ROUTING
bot.on('callback_query', async (query) => {
  ...
  try {
    ...
    // ADMIN HANDLER - UPDATED WITH NEW DEMOTE ALL CALLBACKS
    if (data === 'admin_management' ||
        data === 'add_promote_admin' ||
        // NEW DEMOTE ALL CALLBACKS
        data === 'demote_admin' ||
        data === 'search_demote_groups' ||
        data === 'confirm_demote_all_flow' ||
        data === 'confirm_demote_all_execute' ||
        data.startsWith('select_demote_base_') ||
        data.startsWith('toggle_demote_all_group_') ||
        data.startsWith('demote_all_groups_page_') ||
        // OLD ADD/PROMOTE CALLBACKS
        data === 'search_groups' ||
